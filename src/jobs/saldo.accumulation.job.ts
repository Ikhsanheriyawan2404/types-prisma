...
class SaldoAccumulationJob {
  ...
  private async calculateDailyWages(user: any): Promise<number> {
    const salary: number = Number(user.salary);
    const workingDays: number = user.company?.working_days || 0;
    const dailyWages: number = salary / workingDays;
    const totalDailyWages: number = await db.saldoHistory.count({
      where: {
        user_id: user.id,
        amount: {
          not: 0,
    ...
    return totalDailyWages >= workingDays ? 0 : dailyWages;
  }

  private async resetUserSaldo(user: any): Promise<void> {
